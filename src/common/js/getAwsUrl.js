...
let s3Url = process.env.VUE_APP_S3URL
let s3Url_WithS3Dot = process.env.VUE_APP_S3URL_WITHS3_DOT
let s3Url_WithS3Dash = process.env.VUE_APP_S3URL_WITHS3_DASH
let s3Url_WithS3_Olous = process.env.VUE_APP_S3URL_WITHS3_OLOUS
...
export function getAwsUrl(mediaType, name, is_processed, processed_file) {
  ...
  if (mediaType == media_stored_path.MEDIA_PRODUCT_VIDEO) {
    if (!isContainsStrIgnoreCase(name)) {
      data.awsURL = `${s3Url}media/product/video/${name}`
    } else {
      data.awsURL = name
    }
  }
