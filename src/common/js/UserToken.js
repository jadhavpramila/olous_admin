...
export function createUserTokenCookie(token) {
  ...
  document.cookie = `${user_cookie}=${token}; path=/; expires=${expires}; Domain=${process.env.VUE_APP_USER_TOKEN_DOMAIN}`
...
export function deleteUserTokenCookie() {
  ...
  document.cookie = `${user_cookie}=""; path=/;  expires=${expires}; Domain=${process.env.VUE_APP_USER_TOKEN_DOMAIN}`
}
...
export function updateUserTokenExpiryDate() {
  ...
  if (token_value != '') {
    document.cookie = `${user_cookie}=${token_value}; path=/; expires=${expires}; Domain=${process.env.VUE_APP_USER_TOKEN_DOMAIN}`
  }
}
