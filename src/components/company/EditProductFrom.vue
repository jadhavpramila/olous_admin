<template>
  <div>
    <div v-if="getCurrentEditProductData">
      <div>
        <div class="row">
          <div class="col-md-12">
          ...
          <div class="col-md-4" v-if="hasPrice">
            <div class="form-group">
              ...
              <v-select
                ...
                label="name"
                :searchable="false"
                v-model="productDetails.unit_enum"
                ...
                :class="{
                  'is-invalid':
                    submitted && $v.productDetails.unit_enum.$invalid
                }"
              />
              <!-- <select
                class="form-control input-lg"
            ...
            <div
              v-if="submitted && !$v.productDetails.unit_enum.required"
              class="invalid-feedback d-block"
            >
              Unit is required
            </div>
      ...
      <div>
          ...
          >
            ...
            <div class="product-img-preview">
              ...
              <div class="product-image-overlay px-2">
                ...
                <!-- <label
                  v-if="!img.is_primary && isAddProduct"
                  class="checkbox-container fontSize12 text-white font-weight-600"
                >
                  Make Primary
        ...
        >
          <div class="d-flex">
            <div class="form-group">
              <div>
                <div v-if="productVideo" class="logo-preview">
                  <!-- <img
                  ...
                  /> -->
                  <img
                    @click="showDeleteMediaModal(productVideo.id)"
                    class="remove-product-img"
                    src="../../assets/img/Delete-New.svg"
                    title="close image"
                  />
                  <video v-if="productVideo" height="150" width="100%" controls>
                    <source :src="productVideo.name" />
                  </video>
                </div>
              </div>
            </div>
    ...
    <template>
      ...
      <b-modal ref="mediaModal" hide-footer hide-header :centered="true">
        <div class="d-block">
          ...
          <b-button class="mt-2 btn" variant="outline-dark" @click="deleteImage"
            >Yes</b-button
          >
          <b-button
            class="mt-2 ml-2 btn"
...
export default {
  ...
  methods: {
    ...
    deleteImage() {
      // console.log('id =>', pid)
      let obj = {
        media_id: this.deleteMediaId,
        product_id: this.productDetails.id
      }
      this.hideMediaModal()
      // this.$eventHub.$emit('Cancel_Product_Form', { cancelled: true })
      //console.log('delete media result', obj)
    ...
    getVideoFromMediaList() {
      if (this.getCurrentEditProductData) {
        for (let i = 0; i < this.getCurrentEditProductData.medias.length; i++) {
          if (
            this.getCurrentEditProductData.medias[i].mediaType ==
            appConstants.media_type_enum.VIDEO
          ) {
            this.productVideo = this.getCurrentEditProductData.medias[i]
            this.getCurrentEditProductData.medias.splice(i, 1)
          }
        }
      }
    }
  ...
  mounted() {
    // console.log('getCurrentEditProductData', this.getCurrentEditProductData)
    //if (this.isAddProduct) {
    // }
  ...
  created() {
    ...
    this.getVideoFromMediaList()
    // console.log('getCurrentEditProductData', this.getCurrentEditProductData)
    this.productDetails = this.getCurrentEditProductData
    this.product_images = this.getCurrentEditProductData.medias
    ...
    //   this.product_images = this.mediasData
    if (
      this.getCurrentEditProductData.price !== null &&
      this.getCurrentEditProductData.price !== 0
    ) {
      this.hasPrice = true
    }
  }
