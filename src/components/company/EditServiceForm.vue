<template>
  <div>
    <div v-if="getCurrentEditServiceData">
      <div>
        <div class="row">
          <div class="col-md-12">
          ...
          <div class="col-md-4" v-if="hasPrice">
            <div class="form-group">
              ...
              <v-select
                ...
                label="name"
                :searchable="false"
                v-model="serviceDetails.unit_enum"
                :options="getUnitOptions"
                placeholder="Select"
                :reduce="(unit_enum) => unit_enum.id"
                :closeOnSelect="true"
                class="single-value form-control remove-close-btn show-dropdown-icon"
                :class="{
                  'is-invalid':
                    submitted && $v.serviceDetails.unit_enum.$invalid
                }"
              />
              <!-- <select
                class="form-control input-lg"
            ...
            <div
              v-if="submitted && !$v.serviceDetails.unit_enum.required"
              class="invalid-feedback d-block"
            >
              Unit is required
            </div>
      ...
      <div>
        ...
        >
          ...
          >
            ...
            <div class="product-img-preview">
              ...
              <img
                v-if="serviceDetails.id"
                @click="showDeleteMediaModal(img.id)"
                class="remove-product-img"
        ...
        >
          <div class="d-flex">
            <div class="form-group">
              <div>
                <div v-if="serviceVideo" class="logo-preview">
                  <!-- <img
                  ...
                  /> -->
                  <img
                    @click="showDeleteMediaModal(serviceVideo.id)"
                    class="remove-product-img"
                    src="../../assets/img/Delete-Job.svg"
                    title="close image"
                  />
                  <video v-if="serviceVideo" height="150" width="100%" controls>
                    <source :src="serviceVideo" />
                  </video>
                </div>
              </div>
            </div>
      ...
      <div class="d-flex justify-content-end mt-3">
        ...
        <!-- <span
          ...
          >Save</span
        > -->
        <span class="company-custom-btn ml-4" @click="updateProductInfo"
          >Update</span
        >
      </div>
    </div>
    <template>
      <b-modal ref="imageModal" hide-footer hide-header :centered="true">
        <div class="d-block text-center">
          ...
          <div class="d-flex justify-content-end">
            <span class="mt-2 btn" @click="hideImageModal()">Cancel</span>
            <span class="mt-2 btn ml-2" @click="uploadSingleImage()">Save</span>
          </div>
          <!-- <b-button
            class="mt-2"
      ...
      <b-modal ref="videoModal" hide-footer hide-header :centered="true">
        <div class="d-block text-center">
          ...
          <div class="d-flex justify-content-end">
            <span class="mt-2 btn" @click="hideVideoModal()">Cancel</span>
            <span class="mt-2 ml-2 btn" @click="uploadSingleVideo()">Save</span>
          </div>
        </div>
      </b-modal>
      ...
      <b-modal ref="mediaModal" hide-footer hide-header :centered="true">
        <div class="d-block text-center">
          ...
          <b-button class="mt-2" variant="outline-dark" @click="deleteImage"
            >Yes</b-button
          >
          <b-button
            class="mt-2 ml-2 btn"
...
export default {
  ...
  computed: {
    ...mapGetters([
      ...
      'getCurrentCompanyId',
      'getCurrentEditServiceData'
    ]),
    ...mapGetters('dropdownValues', ['getUnitOptions'])
  },
  methods: {
    ...mapActions([
      'saveServiceInfoNew',
    ...
    setServiceImages(name) {
      let img = `${process.env.VUE_APP_IMGURL}${name}`
      return img
    },
    previewImage() {
    ...
    s3VideoUpload(imageFile) {
      if (imageFile) {
        let imgExt = imageFile.name.substr(imageFile.name.lastIndexOf('.'))
        let fileName = this.$uuid.v1() + imgExt
        this.uploadFileOnS3(
          fileName,
          appConstants.PATH_SAVE_PRODUCT_VIDEO_ON_S3,
          imageFile,
    ...
    removeImageModal() {
      ...
      this.singleVideoData.media_name = ''
    ...
    uploadSingleImage() {
      ...
      this.addSingleMediaImage(payload).then((res) => {
        this.fetchSingleCompanyPreviewDetails(this.getCurrentCompanyId).then(
          () => {
            this.service_images = res
          }
        )
        this.showModal()
        this.service_image = ''
        this.service_video = ''
        this.singleImageData.media_name = ''
        this.singleImageData.media_height = 0.0
        this.singleImageData.media_width = 0.0
    ...
    uploadSingleVideo() {
      ...
      this.addSingleMediaVideo(payload).then((res) => {
        ...
        this.service_video = ''
        this.singleVideoData.media_name = ''
        this.singleVideoData.media_height = 0.0
        this.singleVideoData.media_width = 0.0
    ...
    getPayload(data) {
      ...
      let dataObj = {
        ...
        unit: data.unit_enum == undefined ? '' : data.unit_enum,
    ...
    cancelProduct() {
      this.saveEditCurrentServiceData({})
      this.$eventHub.$emit('Cancel_Service_Form', { cancelled: true })
    ...
    makeImagePrimary(media) {
      let payload = {
        media: media.id,
        product: this.serviceDetails.id,
        company: this.getCurrentCompanyId
      }
      var res = this.service_images.find((item) => item.id === media.id)
      if (res) {
        res.is_primary = true
      }
      var result = this.service_images.find((item) => item.is_primary === true)
      if (result) {
    ...
    getVideoFromMediaList() {
      if (this.getCurrentEditServiceData) {
        for (let i = 0; i < this.getCurrentEditServiceData.medias.length; i++) {
          if (
            this.getCurrentEditServiceData.medias[i].mediaType ==
            appConstants.media_type_enum.VIDEO
          ) {
            this.serviceVideo = this.getCurrentEditServiceData.medias[i]
            this.getCurrentEditServiceData.medias.splice(i, 1)
          }
        }
      }
    }
  ...
  created() {
    this.getVideoFromMediaList()
    this.serviceDetails = this.getCurrentEditServiceData
    this.service_images = this.getCurrentEditServiceData.medias
    ...
    if (
      this.getCurrentEditServiceData.price !== null &&
      this.getCurrentEditServiceData.price !== 0
    ) {
      this.hasPrice = true
    }
  }
