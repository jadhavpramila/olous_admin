<template>
  <div class="cls-analytics form-conatiner">
    <div class="form-container">
      <div class="page-body-sector mt-5">
        <div class="row">
        ...
        <table border="1">
          <thead>
            <tr>
              <td class="font-weight-bold p-2">Sector Title</td>
              <td class="font-weight-bold p-2">User Count</td>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="(profession, index) in userSectorCatagories"
              :key="index"
            >
              <td v-if="profession.key" class="p-2">
                {{ getSectorString(profession.key) }}
              </td>
              <td v-if="profession.key" class="p-2">
...
export default {
    ...
    return {
      ...
      optionsBarChart: {
        chart: {
          ...
          title: {
            text: 'Users count according to Sector',
            align: 'left'
          },
          plotOptions: {
            bar: {
  ...
  computed: {
    ...mapGetters('analytics', ['getter_userSectorAnalytics']),
  ...
  methods: {
    ...
    ...mapActions('analytics', ['action_fetchUserSectorAnalytics']),
    ...
    async clickActiveUserAnalytics() {
      ...
      if (from_date > to_date) {
        ...
        await this.getUserSectorAnalytics(data)
      }
      //console.log('to date', data)
    },

    async getUserSectorAnalytics(data) {
      let obj = {
      ...
      //console.log('obj', obj)
      await this.action_fetchUserSectorAnalytics(obj).then(() => {
        this.userSectorCatagories = {}
        this.userSectorCatagories = this.getter_userSectorAnalytics.users_sectors_categories
    ...
    getSectorString(key) {
      let prof_str = ''
      var arr = this.getSectorOptions.find((x) => x.id == key)
      if (arr != undefined) {
  ...
  async created() {
    ...
    this.getUserSectorAnalytics(data)
  }
}
</script>
